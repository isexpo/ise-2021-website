<div>
    ...
    @endif
    <div class="relative">
        <div class="flex justify-between flex-col md:flex-row items-center mb-1">
            <div class="flex-grow h-10 flex items-center">
                @if($this->searchableColumns()->count())
                    <div class="w-auto flex rounded-lg shadow-sm">
                        <div class="relative flex flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search"
                                   class="form-input focus:outline-none block bg-gray-50 focus:bg-white w-full rounded-md pl-2 transition ease-in-out duration-150 sm:text-sm sm:leading-5 py-2"
                                   placeholder="Search"/>
            ...
            <div class="flex items-center space-x-1">
                ...
                @isset($withDownload)
                    @if($withDownload)
                        <div class="flex flex-column items-start md:items-end mt-3 md:mt-0">
                            <button type="button" wire:click="downloadAll"><label
                            ...
                            @if($isDownloadError)
                                <p class="text-red-500">Tolong pilih data yang ingin diunduh dengan mencentang baris
                                    data</p>
                            @endif
                        </div>
                    @endif
    ...
    @isset($withUbahStatus)
        @if($withUbahStatus)
            <div class="flex md:flex-row flex-col items-center mt-4">
                <h3 class="text-base">Ubah status menjadi</h3> :
                <div class="flex md:flex-row flex-col">
                    @foreach($status as $s)
                        <div class="flex flex-row items-center md:ml-4">
                            <input type="radio" wire:model.defer="statusValue" name="{{$radioName}}" value="{{$s}}"
                                   id="{{$radioName}}-{{$s}}"><label
                                for="{{$radioName}}-{{$s}}" class="ml-1 mb-0">{{$s}}</label>
                        </div>
                    @endforeach

                </div>
                <button class="md:ml-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full"
                        type="button" wire:click="saveStatus">
                    Save
                </button>
            </div>
        @endif
